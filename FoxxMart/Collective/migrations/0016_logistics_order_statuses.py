...
def migrate_processing_orders(apps, schema_editor):
    apps.get_model('Collective', 'CollectiveOrder').objects.filter(status='Payment Confirmed, Processing Order').update(status='Payment confirmed')


class Migration(migrations.Migration):
    dependencies = [('Collective', '0015_collectiveorderitem_size')]
    operations = [
        migrations.RunPython(migrate_processing_orders, migrations.RunPython.noop),
        migrations.AlterField(model_name='collectiveorder', name='status', field=models.CharField(choices=ORDER_STATUSES, max_length=200, null=True)),
    ]
