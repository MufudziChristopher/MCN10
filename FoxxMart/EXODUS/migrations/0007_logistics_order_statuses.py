...
def migrate_processing_orders(apps, schema_editor):
    apps.get_model('EXODUS', 'EXODUSOrder').objects.filter(status='Payment Confirmed, Processing Order').update(status='Payment confirmed')


class Migration(migrations.Migration):
    dependencies = [('EXODUS', '0001_initial')]
    operations = [
        migrations.RunPython(migrate_processing_orders, migrations.RunPython.noop),
        migrations.AlterField(model_name='exodusorder', name='status', field=models.CharField(choices=ORDER_STATUSES, max_length=200, null=True)),
    ]
